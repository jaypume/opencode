...
#### 消息层级与对话轮次 (Message Hierarchy & Conversation Turns)

##### 核心概念

OpenCode 的数据模型采用 **三级层级结构**：

```
Session (会话)
  └── Message[] (消息列表，仅 user / assistant 两种角色)
        └── Part[] (消息片段，12 种类型)
```

**关键设计：没有独立的 "tool" 消息类型。** Tool call 是 Assistant 消息内部的 `ToolPart`，而非单独的消息。这与部分 LLM API（如 Anthropic API 中 `tool_result` 是独立的 user message）不同，OpenCode 做了一层抽象，将工具调用和结果都扁平化为 Part。

##### Message vs Turn（消息 vs 轮次）

| 概念 | 含义 | 粒度 |
|------|------|------|
| **Message** | 一条原子消息，角色只有 `user` 或 `assistant` | 细粒度 |
| **Turn（轮次）** | 一个完整的交互循环：1 个 User Message + 1 个 Assistant Message | 粗粒度 |
| **Part** | 消息内部的内容片段（文本、工具调用、推理等） | 最细粒度 |

##### 一个完整对话轮次的实际结构

```
┌─ User Message (role: "user", id: "msg_001") ──────────────┐
│   parts: [                                                  │
│     TextPart("帮我读一下 config.json 并修复 bug"),              │
│     FilePart(url: "file:///path/to/screenshot.png"),        │
│   ]                                                         │
└─────────────────────────────────────────────────────────────┘

┌─ Assistant Message (role: "assistant", parentID: "msg_001") ┐
│   parts: [                                                   │
│     ReasoningPart("需要先读取文件，分析后再修改..."),               │
│     TextPart("好的，让我先读取文件内容"),                          │
│     ToolPart(tool: "read", state: "completed"),   ← 工具调用 1│
│     TextPart("发现了问题，让我修复它"),                            │
│     ToolPart(tool: "edit", state: "completed"),   ← 工具调用 2│
│     ToolPart(tool: "bash", state: "completed"),   ← 工具调用 3│
│     TextPart("修复完成，测试已通过！"),                            │
│     StepFinishPart(cost: 0.05, tokens: {...}),               │
│     PatchPart(files: ["src/config.ts"]),                     │
│   ]                                                          │
└──────────────────────────────────────────────────────────────┘
```

**要点**:
- 一个 Assistant Message 可以包含 **多个 ToolPart**（对应多次工具调用）
- 所有工具调用的循环（tool call loop）都收纳在 **同一个** Assistant Message 的 parts 数组中
- 每个 ToolPart 有独立的状态机：`pending → running → completed / error`
- `parentID` 将 Assistant Message 链接到触发它的 User Message，建立轮次关系

##### 消息与 AI SDK 的转换

`MessageV2.toModelMessage()` 负责将 OpenCode 内部格式转换为 Vercel AI SDK 格式：

```typescript
// OpenCode 内部格式（1 个 Assistant Message + N 个 Parts）
{
  info: { role: "assistant", parentID: "msg_001" },
  parts: [
    { type: "text", text: "让我读取文件" },
    { type: "tool", tool: "read", callID: "call_1", state: { status: "completed", output: "..." } },
    { type: "text", text: "修复完成" },
  ]
}

// 转换为 AI SDK 格式（展开为多条 model messages）
[
...
这种设计使得 OpenCode 内部保持简洁的 Message → Part 层级，同时对接不同 LLM 提供商时可灵活转换为各自所需的消息格式。

**依据**: `packages/opencode/src/session/message-v2.ts:435-569`
