...
sequenceDiagram
    ...
    participant PLoop as Prompt Loop
    ...
    Prompt->>PLoop: 启动循环(sessionID)
    
    loop 处理循环 (每轮可能多次工具调用)
        PLoop->>PLoop: 检查 Compaction 需求
        PLoop->>PLoop: 获取消息历史
        PLoop->>PLoop: resolveTools() (内置 + MCP)
        
        PLoop->>Proc: SessionProcessor.create()
        Proc-->>PLoop: processor
        
        PLoop->>Proc: processor.process(messages, tools)
        ...
        Proc-->>PLoop: "continue" | "stop" | "compact"
    ...
    PLoop->>Bus: Bus.publish(Session.Event.Idle)
    Bus-->>SDK: SSE Event
    SDK-->>User: 完成
```
